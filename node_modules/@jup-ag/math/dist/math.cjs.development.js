...
var Curve = /*#__PURE__*/function () {
  ...
  _proto.computeY = function computeY(tokenAmounts, inputIndex, outputIndex, newTotalAmount) {
    ...
    for (var i = 0; i < MAX_ITERATIONS; i++) {
      ...
      if (JSBI__default["default"].greaterThan(y, yPrev)) {
        if (JSBI__default["default"].lessThanOrEqual(JSBI__default["default"].subtract(y, yPrev), ONE)) {
          break;
        }
      } else {
        if (JSBI__default["default"].lessThanOrEqual(JSBI__default["default"].subtract(yPrev, y), ONE)) {
          break;
        }
      }
    }
  ...
  _proto.computeD = function computeD(tokenAmounts) {
    ...
    for (var i = 0; i < MAX_ITERATIONS; i++) {
      ...
      if (JSBI__default["default"].greaterThan(d, dPrev)) {
        if (JSBI__default["default"].lessThanOrEqual(JSBI__default["default"].subtract(d, dPrev), ONE)) {
          break;
        }
      } else {
        if (JSBI__default["default"].lessThanOrEqual(JSBI__default["default"].subtract(dPrev, d), ONE)) {
          break;
        }
      }
    }
...
var Stable = /*#__PURE__*/function () {
  ...
  _proto.setTraderFee = function setTraderFee(traderFee) {
    this.traderFee = traderFee;
  };
...
var TokenSwapConstantProduct = /*#__PURE__*/function () {
  ...
  _proto.getPriceImpactExactOutput = function getPriceImpactExactOutput(tokenAmounts, expectedInputTradeAmountJSBI, outputAmountJSBI, inputIndex, outputIndex) {
    if (JSBI__default["default"].equal(outputAmountJSBI, ZERO) || JSBI__default["default"].equal(tokenAmounts[inputIndex], ZERO) || JSBI__default["default"].equal(tokenAmounts[outputIndex], ZERO)) {
      return new Decimal__default["default"](0);
    }
  ...
  _proto.getExpectedInputAmountWithNoSlippage = function getExpectedInputAmountWithNoSlippage(tokenAmounts, outputTradeAmount, inputIndex, outputIndex) {
    if (JSBI__default["default"].equal(tokenAmounts[outputIndex], ZERO)) {
      return tokenAmounts[inputIndex];
    }

    var expectedInputAmountWithNoSlippage = JSBI__default["default"].divide(JSBI__default["default"].multiply(outputTradeAmount, tokenAmounts[inputIndex]), tokenAmounts[outputIndex]);
  ...
  _proto.getAmountPlusFees = function getAmountPlusFees(tradeAmount) {
    return JSBI__default["default"].add(tradeAmount, this.getFees(tradeAmount));
  };
  ...
  _proto.getInputAmount = function getInputAmount(tokenAmounts, outputTradeAmount, inputIndex, outputIndex) {
    ...
    if (JSBI__default["default"].greaterThanOrEqual(outputTradeAmount, poolOutputAmount)) {
      throw new Error('Insufficient liquidity to provide outputTradeAmount');
    }

    var _ceilingDivision2 = ceilingDivision(invariant, JSBI__default["default"].subtract(poolOutputAmount, outputTradeAmount)),
        newPoolInputAmount = _ceilingDivision2[0];

    return JSBI__default["default"].subtract(newPoolInputAmount, poolInputAmount);
  };
