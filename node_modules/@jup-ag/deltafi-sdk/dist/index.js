...
__export(src_exports, {
  checkIfReserveIsSufficient: () => checkIfReserveIsSufficient,
  getDeltafiDexV2: () => getDeltafiDexV2,
  getSwappedAmountsAndPriceImpactFromRawValue: () => getSwappedAmountsAndPriceImpactFromRawValue
});
...
var serumProgramId = new import_anchor.web3.PublicKey("9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin");
function getDeltafiDexV2(programId, provider = null) {
  return new import_anchor.Program(IDL, programId, provider != null ? provider : import_anchor.AnchorProvider.local());
}
...
var WAD_LENGTH = 18;
var WAD = new import_bignumber.default(`1e+${WAD_LENGTH}`);
function BigNumberWithConfig(val, object) {
  const BN2 = import_bignumber.default.clone(object);
  return new BN2(val);
}
...
var ClonedBignumber = import_bignumber.default.clone({
  FORMAT: {
    decimalSeparator: ".",
    groupSeparator: ",",
    groupSize: 3
...
function exponentiate(num, decimals) {
  return new import_bignumber.default(num).multipliedBy(new import_bignumber.default(`1e+${decimals}`));
}
...
function approximateOutAmount(currentReserveA, currentReserveB, targetReserveA, targetReserveB, marketPrice, inputAAmount) {
  ...
  const kMultiplier = kProduct.minus(new import_bignumber3.default(1));
...
function approximateUpperBoundK(currentReserveA, inputAAmount, expCeil) {
  let coreHigh = BigNumberWithConfig(currentReserveA, {
    ROUNDING_MODE: import_bignumber3.default.ROUND_CEIL
  ...
  let coreLow = BigNumberWithConfig(
    ...
    {
      ROUNDING_MODE: import_bignumber3.default.ROUND_FLOOR
    }
...
function calculateOutAmountNormalSwapInternal(marketPrice, targetReserveA, targetReserveB, currentReserveA, currentResreveB, inputAAmount) {
  ...
  if (coreDenumerator.isNegative()) {
    return new import_bignumber4.default(-Infinity);
  }
  let core = BigNumberWithConfig(currentReserveA, {
    ROUNDING_MODE: import_bignumber4.default.ROUND_CEIL
  }).dividedBy(coreDenumerator);
  let exp = BigNumberWithConfig(marketPrice, {
    ROUNDING_MODE: import_bignumber4.default.ROUND_FLOOR
  ...
  let coreExp = BigNumberWithConfig(currentResreveB.toNumber(), {
    ROUNDING_MODE: import_bignumber4.default.ROUND_CEIL
  }).multipliedBy(new import_bignumber4.default(coreExpNumber));
...
function calculateOutAmountNormalSwap(marketPrice, targetReserveA, targetReserveB, currentReserveA, currentReserveB, inputAAmount) {
  ...
  if (inputAAmount.isEqualTo(0)) {
    return { outAmount: new import_bignumber4.default(outputBAmount), priceImpact: new import_bignumber4.default(0) };
  }
  ...
  let actualPrice = new import_bignumber4.default(outputBAmount).dividedBy(inputAAmount);
  let priceImpact = actualPrice.isEqualTo(Infinity) ? new import_bignumber4.default(Infinity) : impliedPrice.minus(actualPrice).dividedBy(actualPrice).abs();
  return { outAmount: new import_bignumber4.default(outputBAmount), priceImpact };
}
function calculateBalancedReservesStableSwap(stablePrice, currentReserveA, currentResreveB, slope) {
  let coefA = new import_bignumber4.default(2).minus(slope).multipliedBy(stablePrice);
  let coefBNeg = new import_bignumber4.default(1).minus(slope).multipliedBy(currentReserveA.multipliedBy(stablePrice).plus(currentResreveB));
  let coefCNeg = slope.multipliedBy(currentReserveA).multipliedBy(currentResreveB);
  let core = BigNumberWithConfig(
    coefBNeg.multipliedBy(coefBNeg).plus(coefA.multipliedBy(coefCNeg).multipliedBy(new import_bignumber4.default(4))),
    {
      ROUNDING_MODE: import_bignumber4.default.ROUND_CEIL
    }
  ).squareRoot();
  let balancedReserveA = BigNumberWithConfig(coefBNeg.plus(core), {
    ROUNDING_MODE: import_bignumber4.default.ROUND_CEIL
  }).dividedBy(coefA).dividedBy(new import_bignumber4.default(2));
...
function calculateOutAmountStableSwapInternal(balancedReserveA, balancedReserveB, currentReserveA, currentResreveB, inputAAmount, slope) {
  let multiplicand = BigNumberWithConfig(
    balancedReserveB.multipliedBy(new import_bignumber4.default(1).minus(slope)),
    {
      ROUNDING_MODE: import_bignumber4.default.ROUND_FLOOR
    }
  ).dividedBy(slope).plus(currentResreveB);
  let coreNumerator = new import_bignumber4.default(1).minus(slope).multipliedBy(balancedReserveA).plus(slope.multipliedBy(currentReserveA));
  let coreDenumerator = new import_bignumber4.default(1).minus(slope).multipliedBy(balancedReserveA).plus(slope.multipliedBy(currentReserveA.plus(inputAAmount)));
  if (coreDenumerator.isLessThanOrEqualTo(0)) {
    return new import_bignumber4.default(-Infinity);
  }
  let multiplier = new import_bignumber4.default(1).minus(
...
function calculateOutAmountStableSwap(stablePrice, currentReserveA, currentReserveB, inputAAmount, slope) {
  ...
  let impliedPrice = currentReserveB.plus(balancedReserveB.multipliedBy(new import_bignumber4.default(1).minus(slope)).dividedBy(slope)).dividedBy(
    currentReserveA.plus(
      balancedReserveA.multipliedBy(new import_bignumber4.default(1).minus(slope).dividedBy(slope))
    )
  );
  ...
  return {
    outAmount: new import_bignumber4.default(outputBAmount.toFixed(0)),
    priceImpact
  };
}
...
var import_client = require("@pythnetwork/client");
...
function getSwapOutAmountSellBase(pool, amountIn, marketPrice) {
  ...
  if (pool.swapType.normalSwap) {
    return calculateOutAmountNormalSwap(
      marketPrice,
      new import_bignumber6.default(pool.poolState.targetBaseReserve.toString()),
      new import_bignumber6.default(pool.poolState.targetQuoteReserve.toString()),
      new import_bignumber6.default(pool.poolState.baseReserve.toString()).plus(virtualBaseReserve),
      new import_bignumber6.default(pool.poolState.quoteReserve.toString()).plus(virtualQuoteReserve),
      amountIn
    );
  } else if (pool.swapType.stableSwap) {
    return calculateOutAmountStableSwap(
      getStableMarketPrice(pool),
      new import_bignumber6.default(pool.poolState.baseReserve.toString()),
      new import_bignumber6.default(pool.poolState.quoteReserve.toString()),
      amountIn,
      new import_bignumber6.default(pool.swapConfig.slope.toString()).dividedBy(WAD)
    );
  } else {
...
function getSwapOutAmountSellQuote(pool, amountIn, marketPrice) {
  ...
  if (pool.swapType.normalSwap) {
    return calculateOutAmountNormalSwap(
      new import_bignumber6.default(1).dividedBy(marketPrice),
      new import_bignumber6.default(pool.poolState.targetQuoteReserve.toString()),
      new import_bignumber6.default(pool.poolState.targetBaseReserve.toString()),
      new import_bignumber6.default(pool.poolState.quoteReserve.toString()).plus(virtualQuoteReserve),
      new import_bignumber6.default(pool.poolState.baseReserve.toString()).plus(virtualBaseReserve),
    ...
    return calculateOutAmountStableSwap(
      new import_bignumber6.default(1).dividedBy(getStableMarketPrice(pool)),
      new import_bignumber6.default(pool.poolState.quoteReserve.toString()),
      new import_bignumber6.default(pool.poolState.baseReserve.toString()),
      amountIn,
      new import_bignumber6.default(pool.swapConfig.slope.toString()).dividedBy(WAD)
...
function getVirtualReserves(swapInfo, marketPrice) {
  const { normalizedBaseReserve, normalizedQuoteReserve } = getNormalizedReserves(
    new import_bignumber6.default(swapInfo.poolState.baseReserve.toString()),
    new import_bignumber6.default(swapInfo.poolState.quoteReserve.toString()),
    new import_bignumber6.default(swapInfo.poolState.targetBaseReserve.toString()),
    new import_bignumber6.default(swapInfo.poolState.targetQuoteReserve.toString()),
    marketPrice
  );
  const virtualReserveRatio = new import_bignumber6.default(swapInfo.swapConfig.virtualReservePercentage).dividedBy(
    100
  );
  return {
...
function getStableMarketPrice(swapInfo) {
  return new import_bignumber6.default(10).pow(swapInfo.mintQuoteDecimals - swapInfo.mintBaseDecimals);
}
