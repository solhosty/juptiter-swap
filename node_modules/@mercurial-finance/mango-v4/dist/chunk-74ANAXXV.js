...
var Group = class {
  ...
  async reloadAlts(client) {
    const alts = await Promise.all(
      this.addressLookupTables.filter((alt) => !alt.equals(_web3js.PublicKey.default)).map(
  ...
  async reloadBanks(client, ids) {
    ...
    if (ids && ids.getBanks().length) {
      banks = (await client.program.account.bank.fetchMultiple(ids.getBanks())).map(
        (account, index) => _chunk772SHZWXjs.Bank.from(ids.getBanks()[index], account)
      );
    } else {
      banks = await client.getBanksForGroup(this);
    }
    const oldbanksMapByTokenIndex = _lodash.cloneDeep.call(void 0, this.banksMapByTokenIndex);
    ...
    for (const bank of banks) {
      ...
      if (oldBanks && oldBanks.length > 0) {
        _lodash.merge.call(void 0, bank, oldBanks[0]);
      }
      const mintId = bank.mint.toString();
      if (this.banksMapByMint.has(mintId)) {
        _optionalChain([this, 'access', _ => _.banksMapByMint, 'access', _2 => _2.get, 'call', _3 => _3(mintId), 'optionalAccess', _4 => _4.push, 'call', _5 => _5(bank)]);
        _optionalChain([this, 'access', _6 => _6.banksMapByName, 'access', _7 => _7.get, 'call', _8 => _8(bank.name), 'optionalAccess', _9 => _9.push, 'call', _10 => _10(bank)]);
        _optionalChain([this, 'access', _11 => _11.banksMapByTokenIndex, 'access', _12 => _12.get, 'call', _13 => _13(bank.tokenIndex), 'optionalAccess', _14 => _14.push, 'call', _15 => _15(bank)]);
      } else {
  ...
  async reloadMintInfos(client, ids) {
    ...
    if (ids && ids.getMintInfos().length) {
      mintInfos = (await client.program.account.mintInfo.fetchMultiple(ids.getMintInfos())).map(
        (account, index) => _chunk772SHZWXjs.MintInfo.from(ids.getMintInfos()[index], account)
      );
    } else {
  ...
  async reloadSerum3Markets(client, ids) {
    ...
    if (ids && ids.getSerum3Markets().length) {
      serum3Markets = (await client.program.account.serum3Market.fetchMultiple(
        ...
        (account, index) => _chunkUGHO4YHYjs.Serum3Market.from(ids.getSerum3Markets()[index], account)
      );
    } else {
  ...
  async reloadSerum3ExternalMarkets(client) {
    const externalMarkets = await Promise.all(
      Array.from(this.serum3MarketsMapByExternal.values()).map(
        (serum3Market) => _serum.Market.load(
  ...
  async reloadPerpMarkets(client, ids) {
    ...
    if (ids && ids.getPerpMarkets().length) {
      perpMarkets = (await client.program.account.perpMarket.fetchMultiple(
        ...
        (account, index) => _chunkZYSI4CYOjs.PerpMarket.from(ids.getPerpMarkets()[index], account)
      );
    } else {
      perpMarkets = await client.perpGetMarkets(this);
    }
    const oldPerpMarketByMarketIndex = _lodash.cloneDeep.call(void 0, 
    ...
    for (const perpMarket of perpMarkets) {
      ...
      if (oldPerpMarket) {
        _lodash.merge.call(void 0, perpMarket, oldPerpMarket);
      }
    }
  ...
  async reloadBankOraclePrices(client) {
    ...
    const coder2 = new (0, _anchor.BorshAccountsCoder)(client.program.idl);
    for (const [index, ai] of ais.entries()) {
      for (const bank of banks[index]) {
        if (bank.name === "USDC") {
          bank._price = _chunkS3PQ6OXSjs.ONE_I80F48.call(void 0, );
          bank._uiPrice = 1;
        } else {
          if (!ai)
            throw new Error(
  ...
  async decodePriceFromOracleAi(coder2, oracle, ai, baseDecimals, client) {
    ...
    if (!_anchor.BorshAccountsCoder.accountDiscriminator("stubOracle").compare(
      ai.data.slice(0, 8)
    )) {
      const stubOracle = coder2.decode("stubOracle", ai.data);
      price = new (0, _chunkS3PQ6OXSjs.I80F48)(stubOracle.price.val);
      uiPrice = this.toUiPrice(price, baseDecimals);
      lastUpdatedSlot = stubOracle.lastUpdated.val;
    } else if (_chunk3ZYUOGD4js.isPythOracle.call(void 0, ai)) {
      const priceData = _client.parsePriceData.call(void 0, ai.data);
    ...
    } else if (_chunk3ZYUOGD4js.isSwitchboardOracle.call(void 0, ai)) {
      const priceData = await _chunk3ZYUOGD4js.parseSwitchboardOracle.call(void 0, 
        ai,
        client.program.provider.connection
      );
  ...
  async reloadVaults(client) {
    ...
    this.vaultAmountsMap = new Map(
      vaultAccounts.map((vaultAi, i) => {
        ...
        const vaultAmount = _token.coder.call(void 0, ).accounts.decode(
          "token",
          vaultAi.data
        ).amount;
  ...
  getTokenVaultBalanceByMintUi(mintPk) {
    ...
    const totalAmount = new (0, _bnjs2.default)(0);
    for (const bank of banks) {
    ...
    return _chunk772SHZWXjs.toUiDecimals.call(void 0, totalAmount, this.getMintDecimals(mintPk));
  }
  ...
  toUiPrice(price, baseDecimals) {
    return _chunk772SHZWXjs.toUiDecimals.call(void 0, price, this.getInsuranceMintDecimals() - baseDecimals);
  }
  toNativePrice(uiPrice, baseDecimals) {
    return _chunk772SHZWXjs.toNativeI80F48.call(void 0, 
      uiPrice,
  ...
  toNativeDecimals(uiAmount, mintPk) {
    ...
    return _chunk772SHZWXjs.toNative.call(void 0, uiAmount, decimals);
  }
  toString() {
    let res = "Group\n";
...
exports.Group = Group;
//# sourceMappingURL=chunk-74ANAXXV.js.map