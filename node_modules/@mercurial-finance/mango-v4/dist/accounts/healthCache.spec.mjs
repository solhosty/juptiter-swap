...
describe("Health Cache", () => {
  it("test_health0", () => {
    ...
    expect(health - (health1 + health2 + health3)).lessThan(1e-7);
  });
  it("test_health1", (done) => {
    function testFixture(fixture) {
      ...
      expect(health - fixture.expectedHealth).lessThan(1e-7);
    }
    const basePrice = 5;
  ...
  it("test_max_swap", (done) => {
    ...
    function findMaxSwapActual(hc2, source, target, minValue, priceFactor, maxSwapFn2) {
      const clonedHc = cloneDeep(hc2);
      const sourcePrice = clonedHc.tokenInfos[source].prices;
      const targetPrice = clonedHc.tokenInfos[target].prices;
      const swapPrice = I80F48.fromNumber(priceFactor).mul(sourcePrice.oracle).div(targetPrice.oracle);
    ...
    function maxSwapFnRatio(hc2) {
      return hc2.healthRatio(HealthType.init);
    }
    function maxSwapFn(hc2) {
      return hc2.health(HealthType.init);
    }
    for (const fn of [maxSwapFn, maxSwapFnRatio]) {
      ...
      }
      {
        ...
        expect(actualRatio / 2 > initRatio);
  ...
  it("test_max_perp", (done) => {
    ...
    hc.tokenInfos[0].balanceNative.iadd(I80F48.fromNumber(3e3));
...
//# sourceMappingURL=healthCache.spec.mjs.map