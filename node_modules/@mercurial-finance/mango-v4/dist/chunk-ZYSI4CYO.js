...
var PerpMarket = class {
  constructor(publicKey, group, settleTokenIndex, perpMarketIndex, groupInsuranceFund, baseDecimals, name, bids, asks, eventQueue, oracle, oracleConfig, stablePriceModel, quoteLotSize, baseLotSize, maintBaseAssetWeight, initBaseAssetWeight, maintBaseLiabWeight, initBaseLiabWeight, openInterest, seqNum, registrationTime, minFunding, maxFunding, impactQuantity, longFunding, shortFunding, fundingLastUpdated, baseLiquidationFee, makerFee, takerFee, feesAccrued, feesSettled, feePenalty, settleFeeFlat, settleFeeAmountThreshold, settleFeeFractionLowHealth, settlePnlLimitFactor, settlePnlLimitWindowSizeTs, reduceOnly, maintOverallAssetWeight, initOverallAssetWeight, positivePnlLiquidationFee) {
    ...
    this.maintBaseAssetWeight = _chunkS3PQ6OXSjs.I80F48.from(maintBaseAssetWeight);
    this.initBaseAssetWeight = _chunkS3PQ6OXSjs.I80F48.from(initBaseAssetWeight);
    this.maintBaseLiabWeight = _chunkS3PQ6OXSjs.I80F48.from(maintBaseLiabWeight);
    this.initBaseLiabWeight = _chunkS3PQ6OXSjs.I80F48.from(initBaseLiabWeight);
    this.baseLiquidationFee = _chunkS3PQ6OXSjs.I80F48.from(baseLiquidationFee);
    this.makerFee = _chunkS3PQ6OXSjs.I80F48.from(makerFee);
    this.takerFee = _chunkS3PQ6OXSjs.I80F48.from(takerFee);
    this.minFunding = _chunkS3PQ6OXSjs.I80F48.from(minFunding);
    this.maxFunding = _chunkS3PQ6OXSjs.I80F48.from(maxFunding);
    this.longFunding = _chunkS3PQ6OXSjs.I80F48.from(longFunding);
    this.shortFunding = _chunkS3PQ6OXSjs.I80F48.from(shortFunding);
    this.feesAccrued = _chunkS3PQ6OXSjs.I80F48.from(feesAccrued);
    this.feesSettled = _chunkS3PQ6OXSjs.I80F48.from(feesSettled);
    this.maintOverallAssetWeight = _chunkS3PQ6OXSjs.I80F48.from(maintOverallAssetWeight);
    this.initOverallAssetWeight = _chunkS3PQ6OXSjs.I80F48.from(initOverallAssetWeight);
    this.priceLotsToUiConverter = new (0, _bigjs2.default)(10).pow(baseDecimals - _chunk772SHZWXjs.QUOTE_DECIMALS).mul(new (0, _bigjs2.default)(this.quoteLotSize.toString())).div(new (0, _bigjs2.default)(this.baseLotSize.toString())).toNumber();
    this.baseLotsToUiConverter = new (0, _bigjs2.default)(this.baseLotSize.toString()).div(new (0, _bigjs2.default)(10).pow(baseDecimals)).toNumber();
    this.quoteLotsToUiConverter = new (0, _bigjs2.default)(this.quoteLotSize.toString()).div(new (0, _bigjs2.default)(10).pow(_chunk772SHZWXjs.QUOTE_DECIMALS)).toNumber();
  }
  ...
  async loadFills(client, lastSeqNum = new (0, _anchor.BN)(0)) {
  ...
  async logOb(client) {
    ...
    let orders = await _optionalChain([this, 'optionalAccess', _ => _.loadAsks, 'call', _2 => _2(client)]);
 ...
 ${order.uiPrice.toFixed(5).padStart(10)}, ${order.uiSize.toString().padStart(10)} ${order.isOraclePegged && order.oraclePeggedProperties ? order.oraclePeggedProperties.pegLimit.toNumber() + " (PegLimit)" : ""}`;
    ...
    res += `
  asks \u2191 --------- \u2193 bids`;
    orders = await _optionalChain([this, 'optionalAccess', _3 => _3.loadBids, 'call', _4 => _4(client)]);
    for (const order of orders.items()) {
      res += `
  ${order.uiPrice.toFixed(5).padStart(10)}, ${order.uiSize.toString().padStart(10)} ${order.isOraclePegged && order.oraclePeggedProperties ? order.oraclePeggedProperties.pegLimit.toNumber() + " (PegLimit)" : ""}`;
    }
  ...
  getCurrentFundingRate(bids, asks) {
    ...
    const bid = bids.getImpactPriceUi(new (0, _anchor.BN)(this.impactQuantity));
    const ask = asks.getImpactPriceUi(new (0, _anchor.BN)(this.impactQuantity));
    ...
    return funding / 24 / Math.pow(10, _chunk772SHZWXjs.QUOTE_DECIMALS);
  }
  uiPriceToLots(price) {
    return _chunk772SHZWXjs.toNative.call(void 0, price, _chunk772SHZWXjs.QUOTE_DECIMALS).mul(this.baseLotSize).div(this.quoteLotSize.mul(new (0, _anchor.BN)(Math.pow(10, this.baseDecimals))));
  }
  uiBaseToLots(quantity) {
    return _chunk772SHZWXjs.toNative.call(void 0, quantity, this.baseDecimals).div(this.baseLotSize);
  }
  uiQuoteToLots(uiQuote) {
    return _chunk772SHZWXjs.toNative.call(void 0, uiQuote, _chunk772SHZWXjs.QUOTE_DECIMALS).div(this.quoteLotSize);
  }
  priceLotsToUi(price) {
    return parseFloat(price.toString()) * this.priceLotsToUiConverter;
  }
  priceNativeToUi(price) {
    return _chunk772SHZWXjs.toUiDecimals.call(void 0, price, _chunk772SHZWXjs.QUOTE_DECIMALS - this.baseDecimals);
  }
  ...
  async getSettlePnlCandidates(client, group, direction, count = 2) {
    ...
    accountsWithSettleablePnl = accountsWithSettleablePnl.filter(
      (acc) => (
        ...
        direction === "negative" && acc.settleablePnl.lt(_chunkS3PQ6OXSjs.ZERO_I80F48.call(void 0, )) || direction === "positive" && acc.settleablePnl.gt(_chunkS3PQ6OXSjs.ZERO_I80F48.call(void 0, ))
      )
    ).sort(
    ...
    if (direction === "negative") {
      ...
      for (let i = 0; i < accountsWithSettleablePnl.length; i++) {
        ...
        const nextPnl = i + 1 < accountsWithSettleablePnl.length ? accountsWithSettleablePnl[i + 1].settleablePnl : _chunkS3PQ6OXSjs.ZERO_I80F48.call(void 0, );
        ...
        perpSettleHealth.gt(_chunkS3PQ6OXSjs.ZERO_I80F48.call(void 0, )) ? (
          // can only settle min
...
var _BookSide = class {
  constructor(client, perpMarket, type, rootFixed, rootOraclePegged, orderTreeNodes, maxBookDelay) {
    ...
    let maxTimestamp = new (0, _anchor.BN)((/* @__PURE__ */ new Date()).getTime() / 1e3 - maxBookDelay);
  ...
  *fixedItems() {
    ...
    while (stack.length > 0) {
      ...
      if (node.tag === _BookSide.INNER_NODE_TAG) {
        ...
        const expiryTimestamp = leafNode.timeInForce ? leafNode.timestamp.add(new (0, _anchor.BN)(leafNode.timeInForce)) : _chunk772SHZWXjs.U64_MAX_BN;
...
var PerpOrder = class {
  ...
  static from(perpMarket, leafNode, type, isExpired = false, isOraclePegged = false) {
    ...
    if (isOraclePegged) {
      ...
      const priceOffset = priceData.sub(new (0, _anchor.BN)(1).ushln(63));
    ...
    return new PerpOrder(
      type === BookSideType.bids ? _chunkS6N5Y2X2js.RUST_U64_MAX.call(void 0, ).sub(leafNode.key.maskn(64)) : leafNode.key.maskn(64),
      leafNode.key,
      leafNode.owner,
      leafNode.ownerSlot,
      0,
...
var _PerpEventQueue = class {
  ...
  eventsSince(lastSeqNum) {
    return this.rawEvents.filter(
      (e) => e.seqNum.gt(lastSeqNum === void 0 ? new (0, _anchor.BN)(0) : lastSeqNum)
...
exports.PerpMarket = PerpMarket; exports.BookSide = BookSide; exports.BookSideType = BookSideType; exports.LeafNode = LeafNode; exports.InnerNode = InnerNode; exports.PerpOrderSide = PerpOrderSide; exports.PerpOrderType = PerpOrderType; exports.PerpOrder = PerpOrder; exports.PerpEventQueue = PerpEventQueue;
//# sourceMappingURL=chunk-ZYSI4CYO.js.map