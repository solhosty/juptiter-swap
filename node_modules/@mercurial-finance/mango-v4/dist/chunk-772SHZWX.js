...
var Bank = class {
  constructor(publicKey, group, name, mint, vault, oracle, oracleConfig, stablePriceModel, depositIndex, borrowIndex, indexedDeposits, indexedBorrows, indexLastUpdated, bankRateLastUpdated, avgUtilization, adjustmentFactor, util0, rate0, util1, rate1, maxRate, collectedFeesNative, loanOriginationFeeRate, loanFeeRate, maintAssetWeight, initAssetWeight, maintLiabWeight, initLiabWeight, liquidationFee, dust, flashLoanTokenAccountInitial, flashLoanApprovedAmount, tokenIndex, mintDecimals, bankNum, minVaultToDepositsRatio, netBorrowLimitWindowSizeTs, lastNetBorrowsWindowStartTs, netBorrowLimitPerWindowQuote, netBorrowsInWindow, borrowWeightScaleStartQuote, depositWeightScaleStartQuote, reduceOnly) {
    ...
    this.oracleConfig = {
      confFilter: _chunkS3PQ6OXSjs.I80F48.from(oracleConfig.confFilter),
      maxStalenessSlots: oracleConfig.maxStalenessSlots
    };
    this.depositIndex = _chunkS3PQ6OXSjs.I80F48.from(depositIndex);
    this.borrowIndex = _chunkS3PQ6OXSjs.I80F48.from(borrowIndex);
    this.indexedDeposits = _chunkS3PQ6OXSjs.I80F48.from(indexedDeposits);
    this.indexedBorrows = _chunkS3PQ6OXSjs.I80F48.from(indexedBorrows);
    this.avgUtilization = _chunkS3PQ6OXSjs.I80F48.from(avgUtilization);
    this.adjustmentFactor = _chunkS3PQ6OXSjs.I80F48.from(adjustmentFactor);
    this.maxRate = _chunkS3PQ6OXSjs.I80F48.from(maxRate);
    this.util0 = _chunkS3PQ6OXSjs.I80F48.from(util0);
    this.rate0 = _chunkS3PQ6OXSjs.I80F48.from(rate0);
    this.util1 = _chunkS3PQ6OXSjs.I80F48.from(util1);
    this.rate1 = _chunkS3PQ6OXSjs.I80F48.from(rate1);
    this.collectedFeesNative = _chunkS3PQ6OXSjs.I80F48.from(collectedFeesNative);
    this.loanFeeRate = _chunkS3PQ6OXSjs.I80F48.from(loanFeeRate);
    this.loanOriginationFeeRate = _chunkS3PQ6OXSjs.I80F48.from(loanOriginationFeeRate);
    this.maintAssetWeight = _chunkS3PQ6OXSjs.I80F48.from(maintAssetWeight);
    this.initAssetWeight = _chunkS3PQ6OXSjs.I80F48.from(initAssetWeight);
    this.maintLiabWeight = _chunkS3PQ6OXSjs.I80F48.from(maintLiabWeight);
    this.initLiabWeight = _chunkS3PQ6OXSjs.I80F48.from(initLiabWeight);
    this.liquidationFee = _chunkS3PQ6OXSjs.I80F48.from(liquidationFee);
    this.dust = _chunkS3PQ6OXSjs.I80F48.from(dust);
    this._price = void 0;
  ...
  getBorrowRate() {
    ...
    if (utilization.gt(this.util1)) {
      ...
      const slope = this.maxRate.sub(this.rate1).div(_chunkS3PQ6OXSjs.I80F48.fromNumber(1).sub(this.util1));
      return this.rate1.add(slope.mul(extraUtil));
    } else if (utilization.gt(this.util0)) {
      const extraUtil = utilization.sub(this.util0);
      const slope = this.maxRate.sub(this.rate0).div(_chunkS3PQ6OXSjs.I80F48.fromNumber(1).sub(this.util0));
...
var MintInfo = class {
  ...
  toString() {
    const res = "mint " + this.mint.toBase58() + "\n oracle " + this.oracle.toBase58() + "\n banks " + this.banks.filter((pk) => pk.toBase58() !== _web3js.PublicKey.default.toBase58()).toString() + "\n vaults " + this.vaults.filter((pk) => pk.toBase58() !== _web3js.PublicKey.default.toBase58()).toString();
    return res;
  }
};

// ts/client/src/utils.ts
var U64_MAX_BN = new (0, _bnjs2.default)("18446744073709551615");
var I64_MAX_BN = new (0, _bnjs2.default)("9223372036854775807").toTwos(64);
function toNativeI80F48ForQuote(uiAmount) {
  return _chunkS3PQ6OXSjs.I80F48.fromNumber(uiAmount * Math.pow(10, 6));
}
function toNativeI80F48(uiAmount, decimals) {
  return _chunkS3PQ6OXSjs.I80F48.fromNumber(uiAmount * Math.pow(10, decimals));
}
function toNative(uiAmount, decimals) {
  return new (0, _bnjs2.default)((uiAmount * Math.pow(10, decimals)).toFixed(0));
}
function toUiDecimals(nativeAmount, decimals) {
  if (nativeAmount instanceof _bnjs2.default) {
    nativeAmount = _chunkS3PQ6OXSjs.I80F48.fromU64(nativeAmount);
  }
  if (nativeAmount instanceof _chunkS3PQ6OXSjs.I80F48) {
    return nativeAmount.div(_chunkS3PQ6OXSjs.I80F48.fromNumber(Math.pow(10, decimals))).toNumber();
  }
...
function toUiI80F48(nativeAmount, decimals) {
  return nativeAmount.div(_chunkS3PQ6OXSjs.I80F48.fromNumber(Math.pow(10, decimals)));
}
async function getAssociatedTokenAddress(mint, owner, allowOwnerOffCurve = false, programId = _spltoken.TOKEN_PROGRAM_ID, associatedTokenProgramId = _spltoken.ASSOCIATED_TOKEN_PROGRAM_ID) {
  if (!allowOwnerOffCurve && !_web3js.PublicKey.isOnCurve(owner.toBuffer()))
    throw new Error("TokenOwnerOffCurve!");
  const [address] = await _web3js.PublicKey.findProgramAddress(
...
async function createAssociatedTokenAccountIdempotentInstruction(payer, owner, mint) {
  ...
  return new (0, _web3js.TransactionInstruction)({
    keys: [
      ...
      {
        pubkey: _web3js.SystemProgram.programId,
        isSigner: false,
        isWritable: false
      },
      { pubkey: _spltoken.TOKEN_PROGRAM_ID, isSigner: false, isWritable: false }
    ],
    programId: _spltoken.ASSOCIATED_TOKEN_PROGRAM_ID,
    data: Buffer.from([1])
  });
}
async function buildVersionedTx(provider, ix, additionalSigners = [], alts = []) {
  ...
  const vTx = new (0, _web3js.VersionedTransaction)(message);
  vTx.sign([
...
exports.U64_MAX_BN = U64_MAX_BN; exports.I64_MAX_BN = I64_MAX_BN; exports.toNativeI80F48ForQuote = toNativeI80F48ForQuote; exports.toNativeI80F48 = toNativeI80F48; exports.toNative = toNative; exports.toUiDecimals = toUiDecimals; exports.toUiDecimalsForQuote = toUiDecimalsForQuote; exports.toUiI80F48 = toUiI80F48; exports.getAssociatedTokenAddress = getAssociatedTokenAddress; exports.createAssociatedTokenAccountIdempotentInstruction = createAssociatedTokenAccountIdempotentInstruction; exports.buildVersionedTx = buildVersionedTx; exports.QUOTE_DECIMALS = QUOTE_DECIMALS; exports.Bank = Bank; exports.MintInfo = MintInfo;
//# sourceMappingURL=chunk-772SHZWX.js.map