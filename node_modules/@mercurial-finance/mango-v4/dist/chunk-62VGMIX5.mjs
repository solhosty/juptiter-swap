// ts/client/src/numbers/I80F48.ts
...
var _I80F48 = class {
  ...
  static fromNumber(x) {
    ...
    v.iadd(new BN((x - int_part) * _I80F48.MULTIPLIER_NUMBER));
  ...
  static fromString(x) {
    const initialValue = new Big(x).times(_I80F48.MULTIPLIER_BIG);
  ...
  toBig() {
    return new Big(this.data.toString()).div(_I80F48.MULTIPLIER_BIG);
  }
  static fromBig(x) {
    return new _I80F48(new BN(x.mul(_I80F48.MULTIPLIER_BIG).round().toFixed()));
  }
  toNumber() {
  ...
  static fromArray(src) {
    ...
    return new _I80F48(new BN(src, "le").fromTwos(_I80F48.MAX_SIZE));
  }
  toArray() {
...
I80F48.MAX_SIZE = 128;
I80F48.FRACTIONS = 48;
I80F48.MULTIPLIER_BIG = new Big(2).pow(_I80F48.FRACTIONS);
I80F48.MULTIPLIER_BN = new BN(2).pow(new BN(_I80F48.FRACTIONS));
I80F48.MULTIPLIER_NUMBER = Math.pow(2, _I80F48.FRACTIONS);
I80F48.MAX_BN = new BN(2).pow(new BN(_I80F48.MAX_SIZE)).div(new BN(2)).sub(new BN(1));
I80F48.MIN_BN = new BN(2).pow(new BN(_I80F48.MAX_SIZE)).div(new BN(2)).neg();
...
//# sourceMappingURL=chunk-62VGMIX5.mjs.map