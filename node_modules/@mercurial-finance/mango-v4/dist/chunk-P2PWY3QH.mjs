...
var MangoAccount = class {
  ...
  isOperational() {
    return this.frozenUntil.lt(new BN(Date.now() / 1e3));
  }
  tokensActive() {
  ...
  getEquity(group) {
    ...
    for (const sp of this.serum3Active()) {
      ...
      tokensMap.get(baseBank.tokenIndex).iadd(I80F48.fromI64(oo.baseTokenTotal).mul(baseBank.price));
  ...
  getMaxWithdrawWithBorrowForToken(group, mintPk) {
    ...
    const expectedVaultMinAmount = tokenBank.nativeDeposits().mul(I80F48.fromNumber(tokenBank.minVaultToDepositsRatio));
 ...
 max token slots ${this.tokens.length}, max serum3 slots ${this.serum3.length}, max perp slots ${this.perps.length}, max perp oo slots ${this.perpOpenOrders.length}`;
    ...
    res = this.perpActive().length > 0 ? res + "\n perps:" + JSON.stringify(
      this.perpActive().map(
        (p) => p.toString(group?.getPerpMarketByMarketIndex(p.marketIndex))
      ),
      null,
      4
    ) : res + "";
...
var _PerpPosition = class {
  ...
  getUnsettledFunding(perpMarket) {
    ...
    if (this.basePositionLots.gt(new BN(0))) {
      return perpMarket.longFunding.sub(this.longSettledFunding).mul(I80F48.fromI64(this.basePositionLots));
    } else if (this.basePositionLots.lt(new BN(0))) {
      return perpMarket.shortFunding.sub(this.shortSettledFunding).mul(I80F48.fromI64(this.basePositionLots));
    }
    return ZERO_I80F48();
  }
  ...
  updateSettleLimit(perpMarket) {
    ...
    const nowTs = new BN(Date.now() / 1e3);
  ...
  toString(perpMarket) {
    return perpMarket ? "market - " + perpMarket.name + ", basePositionLots - " + perpMarket.baseLotsToUi(this.basePositionLots) + ", quotePositive - " + toUiDecimalsForQuote(this.quotePositionNative.toNumber()) + ", bidsBaseLots - " + perpMarket.baseLotsToUi(this.bidsBaseLots) + ", asksBaseLots - " + perpMarket.baseLotsToUi(this.asksBaseLots) + ", takerBaseLots - " + perpMarket.baseLotsToUi(this.takerBaseLots) + ", takerQuoteLots - " + perpMarket.quoteLotsToUi(this.takerQuoteLots) + ", unsettled pnl - " + this.getUnsettledPnlUi(perpMarket).toString() : "";
  }
};
...
var HealthCache = class {
  ...
  computeSerum3Reservations(healthType) {
    const tokenMaxReserved = new Array(this.tokenInfos.length).fill(null).map((ignored) => ZERO_I80F48());
  ...
  perpSettleHealth() {
    ...
    for (const perpInfo of this.perpInfos) {
      const positiveContrib = perpInfo.healthContribution(HealthType.maint).max(ZERO_I80F48());
  ...
  healthRatio(healthType) {
    ...
    if (liabs.gt(I80F48.fromNumber(1e-3))) {
  ...
  adjustPerpInfo(perpInfoIndex, price, side, newOrderBaseLots) {
    if (side == PerpOrderSide.bid) {
      ...
      this.perpInfos[perpInfoIndex].quote.isub(
        I80F48.fromI64(newOrderBaseLots).mul(I80F48.fromI64(this.perpInfos[perpInfoIndex].baseLotSize)).mul(price)
      );
    } else {
  ...
  getMaxSwapSourceForHealthFn(sourceBank, targetBank, price, minFnValue, targetFn) {
    if (sourceBank.initLiabWeight.sub(targetBank.initAssetWeight).abs().lte(ZERO_I80F48())) {
      return ZERO_I80F48();
    }
  ...
  getMaxSerum3OrderForHealthRatio(baseBank, quoteBank, serum3Market, side, minRatio) {
    ...
    if (side == Serum3Side.ask) {
      const quoteBorrows = quote.balanceNative.lt(ZERO_I80F48()) ? quote.balanceNative.abs().mul(quote.prices.liab(HealthType.init)) : ZERO_I80F48();
      ...
      const baseBorrows = base.balanceNative.lt(ZERO_I80F48()) ? base.balanceNative.abs().mul(base.prices.liab(HealthType.init)) : ZERO_I80F48();
    ...
    function cacheAfterPlacingOrder(amount2) {
      const adjustedCache = cloneDeep(healthCacheClone);
      side === Serum3Side.ask ? adjustedCache.tokenInfos[baseIndex].balanceNative.isub(
      ...
      adjustedCache.adjustSerum3Reserved(
        ...
        quoteBank,
        serum3Market,
        side === Serum3Side.ask ? amount2.div(base.prices.oracle) : ZERO_I80F48(),
        ZERO_I80F48(),
        side === Serum3Side.bid ? amount2.div(quote.prices.oracle) : ZERO_I80F48(),
        ZERO_I80F48()
      );
      return adjustedCache;
    }
  ...
  getMaxPerpForHealthRatio(perpMarket, price, side, minRatio) {
    ...
    function healthAfterTrade(baseLots2) {
      return cacheAfterTrade(new BN2(baseLots2.toNumber())).health(
        HealthType.init
      );
    }
    function healthRatioAfterTrade(baseLots2) {
      return cacheAfterTrade(new BN2(baseLots2.toNumber())).healthRatio(
        HealthType.init
      );
    }
    ...
    const hasCase2 = initialBaseLots.gt(ZERO_I80F48()) && direction == -1 || initialBaseLots.lt(ZERO_I80F48()) && direction == 1;
    ...
    if (initialRatio.lte(minRatio) && case1StartRatio.lt(minRatio)) {
      ...
      const startCache = cacheAfterTrade(new BN2(case1Start.toNumber()));
      ...
      const zeroHealthAmount = case1Start.sub(startHealthUncapped.div(finalHealthSlope).div(baseLotSize)).add(ONE_I80F48());
...
var PerpInfo = class {
  ...
  static fromPerpPosition(perpMarket, perpPosition) {
    ...
    const takerQuote = I80F48.fromI64(
      new BN2(perpPosition.takerQuoteLots).mul(perpMarket.quoteLotSize)
    );
...
//# sourceMappingURL=chunk-P2PWY3QH.mjs.map