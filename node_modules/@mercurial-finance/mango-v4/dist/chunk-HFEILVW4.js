...
var _chunk74ANAXXVjs = require('./chunk-74ANAXXV.js');
...
var _chunk3ZYUOGD4js = require('./chunk-3ZYUOGD4.js');
...
var _chunkTBVE5N24js = require('./chunk-TBVE5N24.js');


var _chunkY7A4QF6Jjs = require('./chunk-Y7A4QF6J.js');
...
var _tokeninstructions = require('@project-serum/serum/lib/token-instructions');
...
var MangoClient = class {
  constructor(program, programId, cluster, opts = {}) {
    ...
    this.opts = opts;
    this.idsSource = _optionalChain([opts, 'optionalAccess', _ => _.idsSource]) || "get-program-accounts";
    this.prioritizationFee = _optionalChain([opts, 'optionalAccess', _2 => _2.prioritizationFee]) || 0;
    this.postSendTxCallback = _optionalChain([opts, 'optionalAccess', _3 => _3.postSendTxCallback]);
    this.txConfirmationCommitment = _nullishCoalesce(_nullishCoalesce(_optionalChain([opts, 'optionalAccess', _4 => _4.txConfirmationCommitment]), () => ( program.provider.opts.commitment)), () => ( "processed"));
    Error.stackTraceLimit = 1e3;
  }
  /// Transactions
  async sendAndConfirmTransaction(ixs, opts = {}) {
    return await _chunkA355EMTIjs.sendTransaction.call(void 0, 
      this.program.provider,
      ixs,
      _nullishCoalesce(opts.alts, () => ( [])),
      {
  ...
  async groupEdit(group, admin, fastListingAdmin, securityAdmin, testing, version, depositLimitQuote) {
    const ix = await this.program.methods.groupEdit(
      _nullishCoalesce(admin, () => ( null)),
      _nullishCoalesce(fastListingAdmin, () => ( null)),
      _nullishCoalesce(securityAdmin, () => ( null)),
      _nullishCoalesce(testing, () => ( null)),
      _nullishCoalesce(version, () => ( null)),
  ...
  async ixGateSet(group, ixGateParams) {
    const ix = await this.program.methods.ixGateSet(_chunkWKB2GIFWjs.buildIxGate.call(void 0, ixGateParams)).accounts({
      group: group.publicKey,
      admin: this.program.provider.wallet.publicKey
    }).instruction();
    return await this.sendAndConfirmTransactionForGroup(group, [ix]);
  ...
  async getGroup(groupPk) {
    ...
    const group = _chunk74ANAXXVjs.Group.from(groupPk, groupAccount);
  ...
  async getGroupsForCreator(creatorPk) {
    ...
    const groups = (await this.program.account.group.all(filters)).map(
      (tuple) => _chunk74ANAXXVjs.Group.from(tuple.publicKey, tuple.account)
    );
  ...
  async getGroupForCreator(creatorPk, groupNum) {
    ...
    const filters = [
      ...
      {
        memcmp: {
          bytes: _bs582.default.encode(bbuf),
          offset: 40
        }
      }
    ];
  ...
  async getIds(groupPk) {
    switch (this.idsSource) {
      case "api":
        return await _chunkTBVE5N24js.Id.fromApi(groupPk);
      case "get-program-accounts":
        return void 0;
      case "static":
        return _chunkTBVE5N24js.Id.fromIdsByPk(groupPk);
    }
  }
  // Tokens/Banks
  async tokenRegister(group, mintPk, oraclePk, oracleConfig, tokenIndex, name, interestRateParams, loanFeeRate, loanOriginationFeeRate, maintAssetWeight, initAssetWeight, maintLiabWeight, initLiabWeight, liquidationFee, minVaultToDepositsRatio, netBorrowLimitWindowSizeTs, netBorrowLimitPerWindowQuote) {
    const ix = await this.program.methods.tokenRegister(
      ...
      new (0, _anchor.BN)(netBorrowLimitWindowSizeTs),
      new (0, _anchor.BN)(netBorrowLimitPerWindowQuote)
    ).accounts({
      group: group.publicKey,
      admin: this.program.provider.wallet.publicKey,
      mint: mintPk,
  ...
  async tokenEdit(group, mintPk, params) {
    ...
    const ix = await this.program.methods.tokenEdit(
      ...
      params.netBorrowLimitPerWindowQuote !== null ? new (0, _anchor.BN)(params.netBorrowLimitPerWindowQuote) : null,
      params.netBorrowLimitWindowSizeTs !== null ? new (0, _anchor.BN)(params.netBorrowLimitWindowSizeTs) : null,
      params.borrowWeightScaleStartQuote,
      params.depositWeightScaleStartQuote,
      _nullishCoalesce(params.resetStablePrice, () => ( false)),
      _nullishCoalesce(params.resetNetBorrowLimit, () => ( false)),
      params.reduceOnly
    ).accounts({
      group: group.publicKey,
      oracle: _nullishCoalesce(params.oracle, () => ( bank.oracle)),
      admin: this.program.provider.wallet.publicKey,
  ...
  async tokenDeregister(group, mintPk) {
    const bank = group.getFirstBankByMint(mintPk);
    const adminPk = this.program.provider.wallet.publicKey;
    const dustVaultPk = await _chunk772SHZWXjs.getAssociatedTokenAddress.call(void 0, bank.mint, adminPk);
    ...
    if (!ai) {
      preInstructions.push(
        await _chunk772SHZWXjs.createAssociatedTokenAccountIdempotentInstruction.call(void 0, 
          adminPk,
          adminPk,
          bank.mint
        )
    ...
    const ix = await this.program.methods.tokenDeregister().accounts({
      group: group.publicKey,
      admin: adminPk,
      mintInfo: _optionalChain([group, 'access', _5 => _5.mintInfosMapByTokenIndex, 'access', _6 => _6.get, 'call', _7 => _7(bank.tokenIndex), 'optionalAccess', _8 => _8.publicKey]),
  ...
  async getBanksForGroup(group) {
    ...
    ])).map((tuple) => _chunk772SHZWXjs.Bank.from(tuple.publicKey, tuple.account));
  }
  async getMintInfosForGroup(group) {
    return (await this.program.account.mintInfo.all([
      ...
      return _chunk772SHZWXjs.MintInfo.from(tuple.publicKey, tuple.account);
    });
  }
  async getMintInfoForTokenIndex(group, tokenIndex) {
    ...
    return (await this.program.account.mintInfo.all([
      ...
      {
        memcmp: {
          bytes: _bs582.default.encode(tokenIndexBuf),
          offset: 40
        }
      }
    ])).map((tuple) => {
  ...
  async stubOracleCreate(group, mintPk, price) {
    const ix = await this.program.methods.stubOracleCreate({ val: _chunkS3PQ6OXSjs.I80F48.fromNumber(price).getData() }).accounts({
      group: group.publicKey,
      admin: this.program.provider.wallet.publicKey,
      mint: mintPk,
      payer: this.program.provider.wallet.publicKey
  ...
  async stubOracleSet(group, oraclePk, price) {
    const ix = await this.program.methods.stubOracleSet({ val: _chunkS3PQ6OXSjs.I80F48.fromNumber(price).getData() }).accounts({
      group: group.publicKey,
      admin: this.program.provider.wallet.publicKey,
      oracle: oraclePk
    }).instruction();
  ...
  async getStubOracle(group, mintPk) {
    ...
    return (await this.program.account.stubOracle.all(filters)).map(
      (pa) => _chunk3ZYUOGD4js.StubOracle.from(pa.publicKey, pa.account)
    );
  }
  // MangoAccount
  ...
  async createMangoAccount(group, accountNumber, name, tokenCount, serum3Count, perpCount, perpOoCount) {
    const ix = await this.program.methods.accountCreate(
      _nullishCoalesce(accountNumber, () => ( 0)),
      _nullishCoalesce(tokenCount, () => ( 8)),
      _nullishCoalesce(serum3Count, () => ( 8)),
      _nullishCoalesce(perpCount, () => ( 8)),
      _nullishCoalesce(perpOoCount, () => ( 8)),
      _nullishCoalesce(name, () => ( ""))
    ).accounts({
      group: group.publicKey,
  ...
  async createAndFetchMangoAccount(group, accountNumber, name, tokenCount, serum3Count, perpCount, perpOoCount, loadSerum3Oo = false) {
    const accNum = _nullishCoalesce(accountNumber, () => ( 0));
  ...
  async editMangoAccount(group, mangoAccount, name, delegate) {
    const ix = await this.program.methods.accountEdit(_nullishCoalesce(name, () => ( null)), _nullishCoalesce(delegate, () => ( null))).accounts({
      group: group.publicKey,
  ...
  async getMangoAccount(mangoAccount, loadSerum3Oo = false) {
    const mangoAccountPk = mangoAccount instanceof _chunkC3X3GVS5js.MangoAccount ? mangoAccount.publicKey : mangoAccount;
    const mangoAccount_ = _chunkC3X3GVS5js.MangoAccount.from(
      mangoAccountPk,
      await this.program.account.mangoAccount.fetch(mangoAccountPk)
    );
    if (loadSerum3Oo) {
      await _optionalChain([mangoAccount_, 'optionalAccess', _9 => _9.reloadSerum3OpenOrders, 'call', _10 => _10(this)]);
    }
  ...
  async getMangoAccountWithSlot(mangoAccountPk, loadSerum3Oo = false) {
    ...
    if (!_optionalChain([resp, 'optionalAccess', _11 => _11.value]))
      return;
    const decodedMangoAccount = this.program.coder.accounts.decode(
      "mangoAccount",
      resp.value.data
    );
    const mangoAccount = _chunkC3X3GVS5js.MangoAccount.from(mangoAccountPk, decodedMangoAccount);
    if (loadSerum3Oo) {
      await _optionalChain([mangoAccount, 'optionalAccess', _12 => _12.reloadSerum3OpenOrders, 'call', _13 => _13(this)]);
    }
  ...
  async getMangoAccountsForOwner(group, ownerPk, loadSerum3Oo = false) {
    const accounts = (await this.program.account.mangoAccount.all([
      ...
      return _chunkC3X3GVS5js.MangoAccount.from(pa.publicKey, pa.account);
    });
    if (loadSerum3Oo) {
      await Promise.all(
  ...
  async tokenDeposit(group, mangoAccount, mintPk, amount, reduceOnly = false) {
    ...
    const nativeAmount = _chunk772SHZWXjs.toNative.call(void 0, amount, decimals);
  ...
  async tokenDepositNative(group, mangoAccount, mintPk, nativeAmount, reduceOnly = false) {
    const bank = group.getFirstBankByMint(mintPk);
    const tokenAccountPk = await _chunk772SHZWXjs.getAssociatedTokenAddress.call(void 0, 
      mintPk,
      mangoAccount.owner
    );
    ...
    if (mintPk.equals(_tokeninstructions.WRAPPED_SOL_MINT)) {
      wrappedSolAccount = new (0, _web3js.Keypair)();
      const lamports = nativeAmount.add(new (0, _anchor.BN)(1e7));
      preInstructions = [
        ...
        _tokeninstructions.initializeAccount.call(void 0, {
          account: wrappedSolAccount.publicKey,
          mint: _tokeninstructions.WRAPPED_SOL_MINT,
          owner: mangoAccount.owner
        })
      ];
      postInstructions = [
        _tokeninstructions.closeAccount.call(void 0, {
    ...
    const ix = await this.program.methods.tokenDeposit(new (0, _anchor.BN)(nativeAmount), reduceOnly).accounts({
      group: group.publicKey,
      account: mangoAccount.publicKey,
      owner: mangoAccount.owner,
      bank: bank.publicKey,
      vault: bank.vault,
      oracle: bank.oracle,
      tokenAccount: _nullishCoalesce(_optionalChain([wrappedSolAccount, 'optionalAccess', _14 => _14.publicKey]), () => ( tokenAccountPk)),
  ...
  async tokenWithdraw(group, mangoAccount, mintPk, amount, allowBorrow) {
    const nativeAmount = _chunk772SHZWXjs.toNative.call(void 0, amount, group.getMintDecimals(mintPk));
  ...
  async tokenWithdrawNativeIx(group, mangoAccount, mintPk, nativeAmount, allowBorrow, healthAccountsToExclude = []) {
    ...
    const ix = await this.program.methods.tokenWithdraw(new (0, _anchor.BN)(nativeAmount), allowBorrow).accounts({
      group: group.publicKey,
      account: mangoAccount.publicKey,
      owner: mangoAccount.owner,
      bank: bank.publicKey,
  ...
  async serum3RegisterMarket(group, serum3MarketExternalPk, baseBank, quoteBank, marketIndex, name) {
    const ix = await this.program.methods.serum3RegisterMarket(marketIndex, name).accounts({
      group: group.publicKey,
      admin: this.program.provider.wallet.publicKey,
      serumProgram: _chunkS6N5Y2X2js.OPENBOOK_PROGRAM_ID[this.cluster],
  ...
  async serum3deregisterMarket(group, externalMarketPk) {
    ...
    const [indexReservation] = await _web3js.PublicKey.findProgramAddress(
  ...
  async serum3GetMarkets(group, baseTokenIndex, quoteTokenIndex) {
    ...
    if (quoteTokenIndex) {
      ...
      filters.push({
        memcmp: {
          bytes: _bs582.default.encode(qbuf),
          offset: 42
        }
      });
    }
    return (await this.program.account.serum3Market.all(filters)).map(
      (tuple) => _chunkUGHO4YHYjs.Serum3Market.from(tuple.publicKey, tuple.account)
    );
  }
  ...
  async serum3CloseOpenOrdersIx(group, mangoAccount, externalMarketPk) {
    ...
    const openOrders = _optionalChain([mangoAccount, 'access', _15 => _15.serum3, 'access', _16 => _16.find, 'call', _17 => _17(
      (account) => account.marketIndex === serum3Market.marketIndex
    ), 'optionalAccess', _18 => _18.openOrders]);
  ...
  async serum3PlaceOrderIx(group, mangoAccount, externalMarketPk, side, price, size, selfTradeBehavior, orderType, clientOrderId, limit) {
    ...
    if (!mangoAccount.getSerum3Account(serum3Market.marketIndex)) {
      ...
      if (!_optionalChain([mangoAccount, 'access', _19 => _19.getToken, 'call', _20 => _20(baseTokenIndex), 'optionalAccess', _21 => _21.isActive, 'call', _22 => _22()])) {
        banks.push(group.getFirstBankByTokenIndex(baseTokenIndex));
      }
      if (!_optionalChain([mangoAccount, 'access', _23 => _23.getToken, 'call', _24 => _24(quoteTokenIndex), 'optionalAccess', _25 => _25.isActive, 'call', _26 => _26()])) {
    ...
    const serum3MarketExternalVaultSigner = await _chunkUGHO4YHYjs.generateSerum3MarketExternalVaultSignerAddress.call(void 0, 
      this.cluster,
      serum3Market,
    ...
    const isTaker = orderType !== _chunkUGHO4YHYjs.Serum3OrderType.postOnly;
    const maxQuoteQuantity = new (0, _anchor.BN)(
    ...
    const payerTokenIndex = (() => {
      if (side == _chunkUGHO4YHYjs.Serum3Side.bid) {
    ...
    const ix = await this.program.methods.serum3PlaceOrder(
      ...
      selfTradeBehavior,
      orderType,
      new (0, _anchor.BN)(clientOrderId),
      limit
    ).accounts({
      group: group.publicKey,
      account: mangoAccount.publicKey,
      owner: this.program.provider.wallet.publicKey,
      openOrders: openOrderPk || _optionalChain([mangoAccount, 'access', _27 => _27.getSerum3Account, 'call', _28 => _28(serum3Market.marketIndex), 'optionalAccess', _29 => _29.openOrders]),
      serumMarket: serum3Market.publicKey,
      serumProgram: _chunkS6N5Y2X2js.OPENBOOK_PROGRAM_ID[this.cluster],
      serumMarketExternal: serum3Market.serumMarketExternal,
  ...
  async serum3CancelAllOrders(group, mangoAccount, externalMarketPk, limit) {
    ...
    const ix = await this.program.methods.serum3CancelAllOrders(limit ? limit : 10).accounts({
      ...
      account: mangoAccount.publicKey,
      owner: this.program.provider.wallet.publicKey,
      openOrders: _optionalChain([mangoAccount, 'access', _30 => _30.getSerum3Account, 'call', _31 => _31(serum3Market.marketIndex), 'optionalAccess', _32 => _32.openOrders]),
      serumMarket: serum3Market.publicKey,
      serumProgram: _chunkS6N5Y2X2js.OPENBOOK_PROGRAM_ID[this.cluster],
      serumMarketExternal: serum3Market.serumMarketExternal,
      marketBids: serum3MarketExternal.bidsAddress,
  ...
  async serum3SettleFundsIx(group, mangoAccount, externalMarketPk) {
    ...
    const [serum3MarketExternalVaultSigner, openOrderPublicKey] = await Promise.all([
      _chunkUGHO4YHYjs.generateSerum3MarketExternalVaultSignerAddress.call(void 0, 
        this.cluster,
        serum3Market,
        serum3MarketExternal
      ),
  ...
  async serum3CancelOrderIx(group, mangoAccount, externalMarketPk, side, orderId) {
    ...
    const ix = await this.program.methods.serum3CancelOrder(side, orderId).accounts({
      group: group.publicKey,
      account: mangoAccount.publicKey,
      openOrders: _optionalChain([mangoAccount, 'access', _33 => _33.getSerum3Account, 'call', _34 => _34(serum3Market.marketIndex), 'optionalAccess', _35 => _35.openOrders]),
      serumMarket: serum3Market.publicKey,
      serumProgram: _chunkS6N5Y2X2js.OPENBOOK_PROGRAM_ID[this.cluster],
      serumMarketExternal: serum3Market.serumMarketExternal,
      marketBids: serum3MarketExternal.bidsAddress,
  ...
  async perpCreateMarket(group, oraclePk, perpMarketIndex, name, oracleConfig, baseDecimals, quoteLotSize, baseLotSize, maintBaseAssetWeight, initBaseAssetWeight, maintBaseLiabWeight, initBaseLiabWeight, maintOverallAssetWeight, initOverallAssetWeight, baseLiquidationFee, makerFee, takerFee, feePenalty, minFunding, maxFunding, impactQuantity, groupInsuranceFund, settleFeeFlat, settleFeeAmountThreshold, settleFeeFractionLowHealth, settleTokenIndex, settlePnlLimitFactor, settlePnlLimitWindowSize, positivePnlLiquidationFee) {
    const bids = new (0, _web3js.Keypair)();
    const asks = new (0, _web3js.Keypair)();
    const eventQueue = new (0, _web3js.Keypair)();
    ...
    const ix = await this.program.methods.perpCreateMarket(
      ...
      oracleConfig,
      baseDecimals,
      new (0, _anchor.BN)(quoteLotSize),
      new (0, _anchor.BN)(baseLotSize),
      ...
      minFunding,
      maxFunding,
      new (0, _anchor.BN)(impactQuantity),
      groupInsuranceFund,
      feePenalty,
      settleFeeFlat,
      ...
      new (0, _anchor.BN)(settlePnlLimitWindowSize),
      positivePnlLiquidationFee
    ).accounts({
      group: group.publicKey,
      admin: this.program.provider.wallet.publicKey,
  ...
  async perpEditMarket(group, perpMarketIndex, params) {
    ...
    const ix = await this.program.methods.perpEditMarket(
      ...
      params.minFunding,
      params.maxFunding,
      params.impactQuantity !== null ? new (0, _anchor.BN)(params.impactQuantity) : null,
      params.groupInsuranceFund,
      params.feePenalty,
      ...
      params.settlePnlLimitWindowSize !== null ? new (0, _anchor.BN)(params.settlePnlLimitWindowSize) : null,
      params.reduceOnly,
      _nullishCoalesce(params.resetStablePrice, () => ( false)),
      params.positivePnlLiquidationFee
    ).accounts({
      group: group.publicKey,
      oracle: _nullishCoalesce(params.oracle, () => ( perpMarket.oracle)),
      admin: this.program.provider.wallet.publicKey,
  ...
  async perpGetMarkets(group) {
    ...
    return (await this.program.account.perpMarket.all(filters)).map(
      (tuple) => _chunkZYSI4CYOjs.PerpMarket.from(tuple.publicKey, tuple.account)
    );
  }
  ...
  async perpPlaceOrderIx(group, mangoAccount, perpMarketIndex, side, price, quantity, maxQuoteQuantity, clientOrderId, orderType, reduceOnly, expiryTimestamp, limit) {
    ...
    return await this.program.methods.perpPlaceOrder(
      ...
      maxQuoteQuantity ? perpMarket.uiQuoteToLots(maxQuoteQuantity) : _chunk772SHZWXjs.I64_MAX_BN,
      new (0, _anchor.BN)(clientOrderId ? clientOrderId : Date.now()),
      orderType ? orderType : _chunkZYSI4CYOjs.PerpOrderType.limit,
      reduceOnly ? reduceOnly : false,
      new (0, _anchor.BN)(expiryTimestamp ? expiryTimestamp : 0),
      limit ? limit : 10
    ).accounts({
      group: group.publicKey,
      account: mangoAccount.publicKey,
  ...
  async perpPlaceOrderPeggedIx(group, mangoAccount, perpMarketIndex, side, priceOffset, pegLimit, quantity, maxQuoteQuantity, clientOrderId, orderType, reduceOnly, expiryTimestamp, limit) {
    ...
    return await this.program.methods.perpPlaceOrderPegged(
      ...
      perpMarket.uiBaseToLots(quantity),
      maxQuoteQuantity ? perpMarket.uiQuoteToLots(maxQuoteQuantity) : _chunk772SHZWXjs.I64_MAX_BN,
      new (0, _anchor.BN)(_nullishCoalesce(clientOrderId, () => ( Date.now()))),
      orderType ? orderType : _chunkZYSI4CYOjs.PerpOrderType.limit,
      reduceOnly ? reduceOnly : false,
      new (0, _anchor.BN)(_nullishCoalesce(expiryTimestamp, () => ( 0))),
      limit ? limit : 10,
      -1
    ).accounts({
      group: group.publicKey,
  ...
  async perpCancelOrderIx(group, mangoAccount, perpMarketIndex, orderId) {
    const perpMarket = group.getPerpMarketByMarketIndex(perpMarketIndex);
    return await this.program.methods.perpCancelOrder(new (0, _anchor.BN)(orderId)).accounts({
      group: group.publicKey,
      account: mangoAccount.publicKey,
      owner: this.program.provider.wallet.publicKey,
      perpMarket: perpMarket.publicKey,
  ...
  async perpConsumeEvents(group, perpMarketIndex, accounts, limit) {
    const perpMarket = group.getPerpMarketByMarketIndex(perpMarketIndex);
    const ix = await this.program.methods.perpConsumeEvents(new (0, _anchor.BN)(limit)).accounts({
      group: group.publicKey,
      perpMarket: perpMarket.publicKey,
      eventQueue: perpMarket.eventQueue
    }).remainingAccounts(
  ...
  async perpConsumeAllEvents(group, perpMarketIndex) {
    ...
    while (unconsumedEvents.length > 0) {
      ...
      const accounts = events.map((ev) => {
        switch (ev.eventType) {
          case _chunkZYSI4CYOjs.PerpEventQueue.FILL_EVENT_TYPE: {
            const fill = ev;
            return [fill.maker, fill.taker];
          }
          case _chunkZYSI4CYOjs.PerpEventQueue.OUT_EVENT_TYPE: {
            const out = ev;
            return [out.owner];
          }
          case _chunkZYSI4CYOjs.PerpEventQueue.LIQUIDATE_EVENT_TYPE:
            return [];
          default:
  ...
  async marginTrade({
    ...
    const inputTokenAccountPk = await _chunk772SHZWXjs.getAssociatedTokenAddress.call(void 0, 
      inputBank.mint,
      mangoAccount.owner
    );
    ...
    const outputTokenAccountPk = await _chunk772SHZWXjs.getAssociatedTokenAddress.call(void 0, 
      outputBank.mint,
      mangoAccount.owner
    );
    ...
    const flashLoanBeginIx = await this.program.methods.flashLoanBegin([
      _chunk772SHZWXjs.toNative.call(void 0, amountIn, inputBank.mintDecimals),
      new (0, _anchor.BN)(
        0
      )
    ]).accounts({
      account: mangoAccount.publicKey,
      owner: this.program.provider.wallet.publicKey,
      instructions: _web3js.SYSVAR_INSTRUCTIONS_PUBKEY
    }).remainingAccounts([
      inputBankAccount,
      outputBankAccount,
      inputBankVault,
  ...
  async liqTokenWithToken(group, liqor, liqee, assetMintPk, liabMintPk, maxLiabTransfer) {
    ...
    const ix = await this.program.methods.liqTokenWithToken(assetBank.tokenIndex, liabBank.tokenIndex, {
      val: _chunkS3PQ6OXSjs.I80F48.fromNumber(maxLiabTransfer).getData()
    }).accounts({
      group: group.publicKey,
  ...
  static connect(provider, cluster, programId, opts) {
    const idl = _chunkY7A4QF6Jjs.IDL;
    return new MangoClient(
      new (0, _anchor.Program)(idl, programId, provider),
      programId,
      cluster,
      opts
    );
  ...
  static connectForGroupName(provider, groupName) {
    const idl = _chunkY7A4QF6Jjs.IDL;
    const id = _chunkTBVE5N24js.Id.fromIdsByName(groupName);
    return new MangoClient(
      new (0, _anchor.Program)(
        idl,
        new (0, _web3js.PublicKey)(id.mangoProgramId),
        provider
      ),
      new (0, _web3js.PublicKey)(id.mangoProgramId),
      id.cluster
  ...
  buildFixedAccountRetrieverHealthAccounts(group, mangoAccount, banks, perpMarkets, openOrdersForMarket) {
    ...
    for (const bank of banks) {
      ...
      if (!tokenPositionExists) {
        const inactiveTokenPosition = tokenPositionIndices.findIndex(
          (index) => index === _chunkC3X3GVS5js.TokenPosition.TokenIndexUnset
        );
    ...
    const mintInfos = tokenPositionIndices.filter((tokenIndex) => tokenIndex !== _chunkC3X3GVS5js.TokenPosition.TokenIndexUnset).map((tokenIndex) => group.mintInfosMapByTokenIndex.get(tokenIndex));
    ...
    for (const perpMarket of perpMarkets) {
      ...
      if (!perpPositionExists) {
        const inactivePerpPosition = perpPositionIndices.findIndex(
          (perpIdx) => perpIdx === _chunkC3X3GVS5js.PerpPosition.PerpMarketIndexUnset
        );
    ...
    const allPerpMarkets = perpPositionIndices.filter((perpIdx) => perpIdx !== _chunkC3X3GVS5js.PerpPosition.PerpMarketIndexUnset).map((perpIdx) => group.getPerpMarketByMarketIndex(perpIdx));
    healthRemainingAccounts.push(
    ...
    for (const [serum3Market, openOrderPk] of openOrdersForMarket) {
      ...
      if (!ooPositionExists) {
        const inactiveSerumPosition = serumPositionIndices.findIndex(
          (serumPos) => serumPos.marketIndex === _chunkC3X3GVS5js.Serum3Orders.Serum3MarketIndexUnset
        );
    ...
    healthRemainingAccounts.push(
      ...serumPositionIndices.filter(
        (serumPosition) => serumPosition.marketIndex !== _chunkC3X3GVS5js.Serum3Orders.Serum3MarketIndexUnset
  ...
  buildScanningAccountRetrieverHealthAccounts(group, mangoAccounts, banks, perpMarkets) {
    ...
    if (_optionalChain([banks, 'optionalAccess', _36 => _36.length])) {
      for (const bank of banks) {
...
exports.AccountRetriever = AccountRetriever; exports.MangoClient = MangoClient;
//# sourceMappingURL=chunk-HFEILVW4.js.map